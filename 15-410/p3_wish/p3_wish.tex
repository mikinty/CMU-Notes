...
\maketitle
\te{p3} is the giant of the 15-410 experience, and one of the largest projects you'll ever do at CMU in a class.
...
Maybe it's obvious, but when I started \te{p3} I wasn't sure about the connection
between \te{p3} and \te{p2}. Of course, \te{p2} is user space code, and \te{p3}
is kernel space code, but it seems like you might want to run user space code with kernel code, right?

In \te{p2}, you wrote user code that ran on top of a kernel that was already written for you.
That way, you didn't have to worry about the kernel at all. In \te{p3}, you are implementing
that kernel that ran under your user code. When you are running your kernel in \te{p3}, for the most
part, you don't have to worry about your \te{p2} code. The only thing is probably your 
\te{install\_autostack} (see Plan of Attack \ref{tip:autostack}), which is called on the entry of every program.

What this means, is things like the locks you wrote in \te{p2}
cannot be used by your \te{p3} kernel. This means you will have to rewrite another \te{panic},
and rewrite locks for your \te{p3} kernel.

When you start testing more advanced programs later on, you can run \te{p2} code
on top of your \te{p3} kernel, and see if things still work the way you expect.
...
\section{Locking}
\label{sec:locking}
One trap I fell into while doing \te{p3} was that I wanted to get everything working
with \te{disable\_interrupts} before continuing with locking. While this will
...
In addition, most locking requirements in \te{p3} are not just simple slap-on mutexes,
...
A good start for locking is to port all the fundamental locks you created in \te{p2} -- your 
\te{mutex}, \te{cond}, \te{sem}, \te{rwlock} -- so this way it will encourage you to 
...
\begin{enumerate}
  ...
  in assembly, but using \te{\#define} is a simple way to implement macros for assembly,
  ...
  you should port over all your locking constructs from \te{p2} early on, so you will think 
  ...
  by trying to ``finish'' your kernel first with \te{disable\_interrupts} everywhere.
